...
if (window.location.pathname.includes('dashboard.html')) {
    ...
    function renderPosts(posts) {
        ...
        posts.forEach(post => {
            ...
            row.innerHTML = `
                <td>
                    ${post.image ? `<img src="${API_CONFIG.BASE_URL}${post.image}" alt="${post.title}" class="post-image">` : '<div class="post-image" style="background: #e2e8f0;"></div>'}
                </td>
        ...
        try {
            ...
            if (post.image) {
                currentImage.innerHTML = `
                    <img src="${API_CONFIG.BASE_URL}${post.image}" alt="${post.title}">
